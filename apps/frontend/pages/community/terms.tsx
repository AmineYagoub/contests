import {
  App,
  FindTermsPageQuery,
  FindTermsPageDocument,
  FindTermsPageQueryVariables,
...
export function UserAgreementPage({ data }: { data: App }) {
  appDataVar(data);
  return (
    <StyledSection>
      <Head>
...
export async function getServerSideProps({ req, query }) {
  ...
  try {
    const {
      data: { findAppConfig },
    } = await client.query<FindTermsPageQuery, FindTermsPageQueryVariables>({
      query: FindTermsPageDocument,
    });
...
export default withAuth(UserAgreementPage, null, true);
